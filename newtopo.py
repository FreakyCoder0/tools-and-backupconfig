...
def check_lldp_cdp_enabled(community, ip):
    ...
    cdp_data = snmp_query(community, ip, f'{cdp_base}.6')
    lldp_data = snmp_query(community, ip, f'{lldp_base}.9')
...
def network_discovery(seed_ip, community):
    ...
    def discover_device(ip):
        if not is_valid_ip(ip) or ip in visited_ips:
            return
        neighbor_ip = '' 
        print("Probing Device IP: ", ip)
        device_data = snmp_query(community, ip, '1.3.6.1.2.1.1.5')
        if not device_data:
            return
        ...
        cdp, lldp = check_lldp_cdp_enabled(community, ip)
        if cdp:
            ...
            neighbor_ips = snmp_query(community, ip, neighbor_ip_oid)
            neighbor_names = snmp_query(community, ip, neighbor_name_oid)
            neighbor_ports = snmp_query(community, ip, neighbor_port_oid)
            local_Interfaces = snmp_query(community, ip, localInterface_base)
            device_model = snmp_query(community, ip, neighbor_platform_oid)
        ...
        if lldp:
            ...
            neighbor_names = snmp_query(community, ip, f'{lldp_base}.9') #lldpRemSysName
            neighbor_ports = snmp_query(community, ip, f'{lldp_base}.7')
            local_Interfaces = snmp_query(community, ip, f'{localInterface_base}')
            neighbor_ips = snmp_query(community, ip, f'{neighbourIP_base}.2') #lldpRemManAddr
            device_model = snmp_query(community, ip, f'{lldp_base}.10')
            remoteChassisId = snmp_query(community, ip, f'{lldp_base}.5')

            if len(neighbor_ips): #oid exists in device
                ipFlag =  'lldpRemManAddr'
            else:
                neighbor_ips = snmp_query(community, ip, f'{neighbourIP_base}.4') #lldpRemManAddrIfId, oid has the IP address
                if len(neighbor_ips):
            ...
            for oid, neighbor_name in neighbor_names.items():
                ...
                device_make[neighbor_name] = get_make_model(make_model)
...
if __name__ == "__main__":
    #seed_ip = '172.31.254.2'
    #seed_ip = '22.22.22.2'
    seed_ip = '192.168.100.51'
    community = 'public'
    protocol = 'lldp'  # 'cdp' or 'lldp'

    discovered_devices = network_discovery(seed_ip, community)
